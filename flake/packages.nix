  ...
  perSystem = { pkgs, system, ... }: {
    packages = {
      ...
      migrationScript = pkgs.writeShellScriptBin "activate" ''
        ${self.packages.${system}.default.dependencyEnv}/bin/python -m manage makemigrations
        ${self.packages.${system}.default.dependencyEnv}/bin/python -m manage migrate
        ${self.packages.${system}.default.dependencyEnv}/bin/python -m manage collectstatic --no-input
      '';
